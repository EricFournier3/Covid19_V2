...
import datetime
from lspq_metadata import GetQcMetadataDataFrame
...
parser.add_argument('--metadata-lspq',help="original LSPQ metadata",required=True)
...
metadata_lspq = args.metadata_lspq
...
df_gisaid = pd.read_csv(gisaid_metadata,delimiter="\t",index_col=False,dtype={'length':str})
...
def ConvertFrench2English(countries_list,df_qc):
    ...
    for country in countries_list:
        ...
        #Modif_20201225 add
        english_country = str(translator.translate(country,lang_tgt='en'))
        ...
        if len(check_country_in_lat_long) > 0:
            ...
            df_qc.loc[df_qc['country_exposure'].isin([str(country).capitalize(),str(country).lower(),str(country).upper()]),['country_exposure','division_exposure','rss_exposure','rta_exposure']] = new_country_val
        else:
...
def ImportLatLongCountryNameInMetadata(countries_list,df_qc):
    ...
    for country in countries_list:
        ...
        if len(check_country_in_lat_long) > 0:
            new_country_val = str(check_country_in_lat_long[0])
            df_qc.loc[df_qc['country_exposure'].isin([str(country).capitalize(),str(country).lower(),str(country).upper(),string.capwords(country)]),['country_exposure','division_exposure','rss_exposure','rta_exposure']] = new_country_val
...
def CheckMissingCountry(df_qc):
    ...
    locs_in_lspq_orig = [x for x in df_qc['country_exposure'].unique() if x != "Canada"]
    #print(locs_in_lspq_orig)
    ...
    #print("MISSING ",missing_latlong_locs)

    #Modif_20201222 temporaire voir /data/Applications/GitScript/Covid19_V2/NextStrainFiles/scripts/NOTE_20201222.txt
    still_missing_country_in_latlong = ConvertFrench2English(missing_latlong_locs,df_qc)
    #still_missing_country_in_latlong = []

    present_country = [loc for loc in locs_in_lspq if loc in locs_w_latlong if loc != "Canada"]
...
df_qc_from_dsp_db = GetQcMetadataDataFrame(qc_seq_id,metadata_lspq)

CheckMissingRTA(df_qc_from_dsp_db)

missing_country = CheckMissingCountry(df_qc_from_dsp_db)

#print(df_qc_from_dsp_db)
...
#print("Date length ", str(subset_gisaid_subcol['date'].str.len().values))
#for j in subset_gisaid_subcol['date'].str.len().values:
#    if j not in [4,7,10]:
#        print("Date Length ",j)
subset_gisaid_subcol.loc[subset_gisaid_subcol['date'].str.len() == 7,['date']] =  subset_gisaid_subcol['date'] + "-01"
subset_gisaid_subcol = subset_gisaid_subcol.loc[subset_gisaid_subcol['date'].str.len() != 4,:]
...
subset_gisaid_final['OUTBREAK'] = "NA"
