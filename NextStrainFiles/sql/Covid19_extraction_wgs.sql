...
( select    
    'res_genotype'
    from RESULTS coro
    where coro.ORDNO = ot.ORDNO  AND coro.TESTNO='COVID-19 - Surveillance WGS'  AND coro.ANALYTE='Génotype'  and coro.REPORTABLE = 'Y' 
...
( select 
    '0'
    from RESULTS res
    where res.ORDNO = ot.ORDNO  AND res.TESTNO='COVID-19 - Surveillance WGS'  AND res.ANALYTE='Génotype'  
) as nCoV2019_nCoV_Ct
