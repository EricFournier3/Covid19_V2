select distinct FOLDERNO as NUMERO_SGIL , to_char(cr.DATE_COLLECTED,'YYYY-MM-DD') as SAMPLED_DATE, cr.LAST_NAME as NOM, cr.FIRST_NAME as PRENOM, to_char(cr.BIRTH_DATE,'YYYY-MM-DD') as DATE_NAISS, 
PID,REPLACE(REPLACE(adress || ' ' || adress_a || ', '|| CITY ||', QC, Canada',CHR(10),' '),CHR(13),' ') as CH_ADRESS, COMPNAME as CH_NAME
