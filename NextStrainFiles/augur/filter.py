...
def dedup(group_seq_id,all_nuc_seqs,max_dup):

    dedup_rec_dict = {}
    dedup_recid_list = []

    group_seq_nuc = [seq for id,seq in all_nuc_seqs.items() if id in group_seq_id]
    for rec in group_seq_nuc:
        checksum = seguid(str.upper(str(rec.seq)))
        if checksum in dedup_rec_dict:
            nb_rec = len(dedup_rec_dict[checksum])
            #remplacer 2 par le user parameter
            if(nb_rec < max_dup):
                dedup_rec_dict[checksum].append(rec.id)
                dedup_recid_list.append(rec.id)
        else:
            dedup_rec_dict[checksum] = []
            dedup_rec_dict[checksum].append(rec.id)
            dedup_recid_list.append(rec.id)

    return(dedup_recid_list.copy())
...
def register_arguments(parser):
    ...
    parser.add_argument('--max-date', type=str, help="maximal cutoff for numerical date")
    ...
    #--all-canadian-seq
    parser.add_argument('--all-canadian-seq',help="include all canadian sequences",action='store_true')

    parser.add_argument('--max-dup-per-group',type=int,help="maximal number of duplicate sequence in same group")
...
def run(args):
    ...
    if args.group_by and args.sequences_per_group:
        ...
        if len(seq_names_by_group)==1 and ('unknown' in seq_names_by_group or ('unknown',) in seq_names_by_group):
            ...
            if(args.all_canadian_seq):
                to_delete = [mykey for mykey in seq_names_by_group if re.search(r'Canada',str(mykey))]
                for mykey in to_delete: del seq_names_by_group[mykey]


            if(args.max_dup_per_group):
                for group,sequences_in_group in seq_names_by_group.items():
                    seq_names_by_group[group] = dedup(sequences_in_group,seqs,args.max_dup_per_group)
                
            for group, sequences_in_group in seq_names_by_group.items():
                if args.priority: #sort descending by priority
                    my_seq_ids = sorted(sequences_in_group, key=lambda x:priorities[x], reverse=True)[:spg]
