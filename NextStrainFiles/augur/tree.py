...
def GetNumberOfRaxML_Core(aln_file):
    nb_rec = len([rec for rec in Bio.SeqIO.parse(aln_file,"fasta")])

    if nb_rec < 69:
        return "2"
    elif nb_rec in range(69,225):
        return "4"
    elif nb_rec > 225:
        return "20"
...
def build_raxml(aln_file, out_file, clean_up=True, nthreads=1, tree_builder_args=""):
    ...
    nb_process = GetNumberOfRaxML_Core(aln_file)
 
    #call = ["mpirun -np", nb_process ,"raxml-ng-mpi ", "--msa-format FASTA ", "--msa ",shquote(aln_file)," --model GTR+G6+I "," --prefix RAxML", " --threads 1 ","--extra thread-pin" ]
    call = ["mpirun -np", nb_process ,"raxml-ng-mpi ", "--msa-format FASTA ", "--msa ",shquote(aln_file)," --model GTR "," --prefix RAxML", " --threads 1 ","--extra thread-pin" ]
    cmd = " ".join(call)
    ...
    print("Building a tree via:\n\t" + cmd +
          "\n\tKozlov, A.M and Statmatakis, A: RAxML-NG: Version 9: a fast, scalable and user-friendly tool for maximum likelihood phylogenetic inference"
          "\n\tIn Bioinformatics, 2019\n")
    try:
        run_shell_command(cmd, raise_errors = True)
        #RAxML.raxml.bestTree    
        #EricF comment
        #shutil.copy("RAxML_bestTree.%s"%(random_string), out_file)
        shutil.copy("RAxML.raxml.bestTree", out_file)
        ...
        if clean_up:
            ...
            os.remove("RAxML.raxml.rba")
            os.remove("RAxML.raxml.startTree")
            os.remove("RAxML.raxml.mlTrees")
            os.remove("RAxML.raxml.log")
            os.remove("RAxML.raxml.bestModel")
        ...
        if os.path.isfile("RAxML.raxml.log"):
             print("Please see the log file for more details: {}".format("RAxML.raxml.log"))
        T=None
